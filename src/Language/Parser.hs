...
import Language.ParserUtils (Parser, apply, empty, ident, ior, lit, num, oneOrMore, oneOrMoreWithSep, relOps, satisfies, then2, then3, then4, unwrap, xor, zeroOrMore)
import Language.Syntax (CoreAlternative, CoreDefinition, CoreExpr, CoreProgram, CoreSupercombinator, Expr (EApp, ECase, EConstr, ELam, ELet, ENum, EVar), IsRec, PartialExpr (FoundOp, NoOp), nonRecursive, recursive)
import Language.Token (Token)
...
parseExpr4c =
  (then2 FoundOp (lit "+") parseExpr4 `xor` empty NoOp)
    `xor` (then2 FoundOp (lit "-") parseExpr5 `xor` empty NoOp)
...
parseExpr5c =
  (then2 FoundOp (lit "*") parseExpr5 `xor` empty NoOp)
    `xor` (then2 FoundOp (lit "/") parseApplication `xor` empty NoOp)
